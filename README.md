# SCX Plugin for Claude Code

Claude Code skills for managing SCX.ai models and delegating coding tasks to a
sub-agent running on SCX Cloud.
...
- A **`SCX_API_KEY`** environment variable — required by `/model-info` and `/code`.
...
There is **no manual setup step beyond `/setup`**. On each session start the plugin
...
| [model-info](#model-info) | `/model-info` | Show all models available on the SCX platform |
...
After this, all other skills are ready to use.
...
|---|---|---|
| `model` | Yes | Bare model ID from the database (e.g. `MiniMax-M2.7`). Use `/list-models` to see options. |
...
| `--session` | No | Resume a previous `/code` session to preserve context. Resuming must use the **same `cwd`** as the original call. |
