...
try:
    from agent_shims.environment import get_scx_base_url, get_scx_key
...
USER_AGENT = "scx-plugin"

# Different SCX-compatible endpoints name the same concept differently.
...
def _run_and_format(m: Model, prompt: str, cwd: str, args: list[str],
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(
            ...
            f"(baseURL={get_scx_base_url()}, model={m.id})\n{err}\n"
            "If this is a model-not-found / auth error, the model may not be "
            "served on this endpoint — set SCX_BASE_URL to the "
...
async def model_info() -> str:
    """Show all models on the SCX platform + context length + max completion
    ...
    SCX_API_KEY in the environment.
    """
