...
allowed-tools: mcp__plugin_scx-plugin_scx-plugin__code, mcp__plugin_scx-plugin_scx-plugin__list_models, Write, Read
---
...
`mcp__plugin_scx-plugin_scx-plugin__code`. DO NOT pass raw
user text — shape it into a proper prompt per the guidelines below.

Requires the `scx-plugin` MCP server to be running.

### MCP tool arguments

| Arg | Required | Description |
|---|---|---|
| `model` | yes | Model name from parameters database (use `/list-models`). **Important:** bare model ID (e.g. `MiniMax-M2.7`), not provider-prefixed. Provider auto-configured. |
